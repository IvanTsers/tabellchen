...
#+end_src
#+begin_export latex
\subsection{Function \texttt{DistinctByCol}}
!The function \texttt{DistinctByCol} returns a \texttt{cond} filtering
!function that checks if a string is unique. The first occurence of a
!string is always kept.

We initialize a map of seen strings. In the produced function, we
check if the current string has been seen already. If not so, we add
it to the map.
#+end_export
#+begin_src go <<Functions>>=
